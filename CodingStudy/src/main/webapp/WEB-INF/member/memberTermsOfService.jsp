...
제 1 조 (목 적)
이 약관은 이용자가 주식회사 다락(이하 “회사”)이 운영하는 인터넷 서비스 사이트(이하 “사이트” 또는 “코딩쌤”)를 통해 제공하는 인터넷 전자상거래 관련 서비스(이하 “서비스”)와 관련하여 회사와 이용자의 권리, 의무, 책임사항을 규정함을 목적으로 합니다. 또한 본 약관은 유무선 PC통신, 태블릿 PC, 스마트폰(아이폰, 안드로이드폰 등) 어플리케이션 및 모바일웹 등을 이용하는 전자상거래 등에 대해서도 그 성질에 반하지 않는 한 준용됩니다.
본 약관이 규정한 내용 이외의 회사와 이용자 간의 권리, 의무 및 책임사항에 관하여서는 전기통신사업법 기타 대한민국의 관련 법령과 상관습에 의합니다.
제 2 조 (정 의)
이 약관에서 사용하는 용어의 정의는 다음과 같습니다.

회사가 운영하는 사이트는 아래와 같습니다.
codingssam.com, 추후 회사에서 공지하고 제공하는 기타 웹사이트, 스마트폰 및 이동통신기기를 통해 제공되는 모바일 어플리케이션 포함
이용자: 코딩쌤에 접속하여 이 약관에 따라 서비스를 받는 회원 및 비회원을 말합니다.
회원: 본 약관에 따라 회사와 서비스 이용계약을 체결하고 회원 아이디(ID)를 부여받아 회사가 제공하는 서비스를 이용하는 개인을 말합니다. 회원은 이용하는 서비스에 따라 고객과 선생님 회원으로 나누어집니다.
...
선생님 회원: 회사에 개인정보를 제공하여 회원등록을 한 자로서, 회사와 서비스 이용계약을 체결하고 회원 아이디(ID)를 부여 받아 고객을 찾기 위한 서비스를 계속적으로 이용할 수 있는 자를 말합니다.

신청서: 신청자가 원하는 용역에 대한 구체적인 정보를 담고 있는 서비스 요청서입니다.

콘텐츠(Contents): 회사가 코딩쌤에서 제공하는 정보, 요청서 작성, 견적 작성, 요청서 수신, 견적 수신, 프로그램 등 부호ㆍ문자ㆍ도형ㆍ색채ㆍ음성ㆍ음향ㆍ이미지ㆍ영상 및 복합체의 정보나 자료를 의미합니다.
...
비회원: 회원에 가입하지 않고 서비스를 이용하는 자로 코딩쌤 이용자 및 정보의 수신을 위해 전자우편 및 SMS 서비스를 온라인 또는 서면으로 신청한 자를 지칭합니다.
회원 탈퇴: 회원이 이용계약을 종료시키는 행위를 말합니다.
뉴스레터(Newsletter): 회사가 보내는  소식이 담긴 이메일을 의미합니다.

인터랙션 : 회원간의 채팅, 전화 등 코딩쌤에서 제공하는 방식을 통해 상호작용 할 수 있는 행위를 의미 합니다.
마켓 : 고수 회원이 자신이 제공할 수 있는 유형의 제품 또는 무형의 용역에 대한 금액과 내용을 회사가 제공하는 일정한 양식에 따라 회사의 사이트에 등록하고, 고객이 정해진 금액을 결제하여 선생님 회원으로부터 유형의 제품 또는 무형의 용역을 제공 받는 형태의 서비스를 말합니다.
제 3 조 (약관의 명시, 효력 및 개정)
회사는 이 약관의 내용과 주소지, 관리자의 성명, 개인정보보호 담당자의 성명, 연락처(전화, 팩스, 전자우편 주소 등) 등을 이용자가 알 수 있도록 코딩쌤의 초기 서비스 화면에 게시합니다. 다만, 약관의 구체적 내용은 이용자가 연결화면을 통하여 볼 수 있습니다.
이 약관은 그 내용을 코딩쌤에 게시하거나 이메일 등 기타의 방법으로 회원에게 공지함으로써 효력이 발생합니다.
...
코딩쌤 서비스 이용은 사이트의 업무상 또는 기술상 특별한 지장이 없는 한 연중무휴, 1일 24시간을 원칙으로 합니다.
회사가 코딩쌤에서 제공하는 서비스는 기본적으로 무료입니다. 
...
제 12 조 (회원탈퇴 및 자격상실)
회원이 이용계약을 해지하고자 하는 때에는 회원 본인이 코딩쌤 내의 메뉴 또는 전화 등의 방법을 이용하여 가입해지를 신청할 수 있으며, 회사는 즉시 회원 탈퇴 처리를 합니다.
...
회원의 귀책사유에 따른 이용계약의 해제·해지시 잔여 보너스 캐시는 소멸합니다. 잔여 충전 캐시가 있을 경우 회사는 회원가입시 회원이 입력한 연락처로 최소 30일의 기간을 두고 잔여 캐시 환불을 위한 계좌정보 등을 입력할 것을 최고합니다. 회원이 최고 기간 내 환불을 위한 정보를 입력하지 않을 경우 잔여 캐시는 소멸합니다. "
...
회사의 개인정보 보호는 코딩쌤 개인정보처리방침에 따릅니다.
...
제1호 내지 제12호 기타 부정한 방법으로 코딩쌤 캐시를 적립, 사용한 경우
...
타 회원들에게 허위사실을 유포하거나 표시광고를 위반하는 경우(견적에 회사에 상당한 수수료를 지불하고 있어, 절대 채팅으로 답변하지 말라는 내용을 기재하거나, 명함에 코딩쌤 로고를 무단으로 표시하고 자신을 코딩쌤 협력사로 소개하는 등을 포함함): 모든 기능 사용 정지(영구 제재)
...
고수 회원은 견적을 수령한 고객과의 코딩쌤 서비스에서 제공하는 메시지 기능이나 전화 등 별도의 연결수단을 통하여 세부사항을 조정할 수 있습니다. 회사는 견적 발송 이후 고수 회원과 고객 간에 발생한 일체의 문제에 대하여 어떠한 책임도 지지 않습니다.
...
가. 선생님의 프로필 정보
나. 요청자 작성한 후기(이에 포함된 글 및 사진)
다. 요청서 및 견적의 내용
라. 회사와 요청자 또는 선생님 사이의 연락 내용
마. 기타 요청자 및 고수가 서비스 이용과정에서 작성·게시한 콘텐츠
제 23 조 (광고주 및 연결 사이트와의 관계)
회사의 공식 사이트 이외의 웹사이트 및 이메일에서 링크된 사이트에서는 회사의 개인정보 보호정책 및 본 약관이 적용되지 않습니다.
회사는 코딩쌤 사이트 또는 e-mail 상에 게재되어 있거나 본 서비스를 통한 광고주의 판촉활동에 회원이 참여하거나 교신 또는 거래를 함으로써 발생하는 손실과 손해에 대해 책임을 지지 않습니다.
...
회사는 회원들에게 요청서와 견적을 전달하거나 광고플랫폼 또는 페이 서비스를 제공하고 이에 대한 부수적인 서비스를 제공할 뿐이므로 회원 상호 간 서비스를 매개로 하여 거래를 한 경우 이용자 각자의 의무이행에 대한 책임은 각 이용자에게 있으며 회사는 이에 대한 책임이 없습니다.
회사는 회원들에게 요청서와 견적을 전달하거나 광고플랫폼 또는 페이 서비스를 제공하고 이에 대한 부수적인 서비스를 제공할 뿐이므로 고수와 요청자 간 거래와 관련하여 발생한 의무의 불완전 이행, 이행 지체 등 용역 이행 미비, 사후처리, 대금 정산, 완성품의 하자, 청약철회, 물품의 반품 등 어떠한 법적 분쟁 및 사후처리에 대해서도 개입하거나 책임지지 않습니다.
...
부칙
제1조 (시행일) 이 약관은 2022년 07월 01일부터 시행합니다.
